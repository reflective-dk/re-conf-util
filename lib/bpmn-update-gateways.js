...
module.exports = bpmnUpdateGateways;

function bpmnUpdateGateways(xml, objectMap, cb) {
    ...
    moddle.fromXML(xml, function(err, definitions) {
        parser.parseString(xml, function(err, rawdef) {
            ...
            var report = updateGateways(bpmnProcess, _.get(rawdef, [ 'bpmn:process', 0 ]), objectMap);
...
//
// Update / add gateway expresion, based on the lanes responsibility extension
//
function updateGateways(process, rawProcess, objectMap) {
console.log("hello process=",process);
    var flowNodeRefs = {};
    var invalidElements = [];
    var register = {
        userTasks: {},
...
function xmlVisitLanes(element, visitor, path) {
    ...
    _.get(element, 'bpmn:laneSet', []).forEach(laneSet => xmlVisitLanes(laneSet, visitor, extendPath(laneSet, path)));
    _.get(element, 'bpmn:childLaneSet', []).forEach(laneSet => xmlVisitLanes(laneSet, visitor, extendPath(laneSet, path)));
    _.get(element, 'bpmn:lane', []).forEach(lane => xmlVisitLanes(lane, visitor, extendPath(lane, path)));
}

function extendPath(element, path) {
    return path + '/' + ((element || {}).id || 'INVALID');
}

...
const fs = require('fs');
const mockTestDataServer = 'http://mock-response.reflective.dk/test/data/';
...
async function exposeCallbacks(page) {
  ...
  await page.exposeFunction(LOG_CB, (context) => {
    try {
      ...
      logger.debug('log', JSON.stringify(record));
...
async function runQunitPuppeteer(qunitPuppeteerArgs) {
  ...
  if (qunitPuppeteerArgs.executablePath) {
    args.executablePath = qunitPuppeteerArgs.executablePath;
  }
  ...
  try {
    ...
    page.on('console', (consoleArgs) => {
      if (consoleArgs.type() === 'error') {
        logger.error(consoleArgs.text(), consoleArgs.location());
      }
      if (qunitPuppeteerArgs.redirectConsole) {
        Console.log('[%s] %s', consoleArgs.type(), consoleArgs.text());
      }
    });
    ...
    page.on('request', (request) => {
      let url = request.url();
      logger.debug('requesting a file', url);
      if (url.indexOf(mockTestDataServer) === 0) {
        let result = fs.readFileSync(qunitPuppeteerArgs.testDataDir + url.split(mockTestDataServer)[1]);
        request.respond({
    ...
    const evaluateArgs = {
      mockTestDataServer: mockTestDataServer,
      testTimeout: timeout,
      callbacks: {
        begin: BEGIN_CB,
        done: DONE_CB,
    ...
    await page.evaluateOnNewDocument((qunitConfiguration) => {
      ...
      function extendQUnit(QUnit) {
        try {
          ...
          window.testDataDir = qunitConfiguration.mockTestDataServer;
